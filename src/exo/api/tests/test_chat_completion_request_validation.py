...
defends against pathological values up-front so callers see a structured
422 instead of an opaque mid-stream error.
"""
...
def test_num_draft_tokens_pathological_value_rejected() -> None:
    """The K=8 case that killed the runner is well below the cap, but the
    cap exists to reject genuinely malformed values like 1024."""
    with pytest.raises(ValidationError):
        ChatCompletionRequest.model_validate(_minimal_payload(num_draft_tokens=1024))
