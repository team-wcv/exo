# pyright: reportPrivateUsage=false

from pytest import MonkeyPatch

from exo.utils.info_gatherer.net_profile import is_probeable_interface


def test_probeable_interface_rejects_non_lan_paths(monkeypatch: MonkeyPatch):
    monkeypatch.delenv("EXO_REACHABILITY_ALLOWED_CIDRS", raising=False)

    assert is_probeable_interface("en11", "192.168.1.63")
    assert not is_probeable_interface("lo0", "127.0.0.1")
    assert not is_probeable_interface("en5", "169.254.108.157")
    assert not is_probeable_interface("utun0", "100.80.253.10")
    assert not is_probeable_interface("utun0", "fd7a:115c:a1e0::9938:fd0a")
    assert not is_probeable_interface("awdl0", "fe80::1406:6dff:fed0:b945%awdl0")


def test_probeable_interface_admits_only_verified_apple_ncm_link_local(
    monkeypatch: MonkeyPatch,
):
    monkeypatch.delenv("EXO_REACHABILITY_ALLOWED_IPS", raising=False)
    monkeypatch.delenv("EXO_REACHABILITY_ALLOWED_CIDRS", raising=False)

    assert is_probeable_interface(
        "en9", "169.254.49.224", interface_type="apple_usb_ncm"
    )
    assert not is_probeable_interface(
        "en9", "169.254.49.224", interface_type="ethernet"
    )
    assert not is_probeable_interface(
        "en9", "fe80::c4f:acbe:e0e0:9c3b%en9", interface_type="apple_usb_ncm"
    )


def test_apple_ncm_link_local_still_honors_operator_cidr(
    monkeypatch: MonkeyPatch,
):
    monkeypatch.delenv("EXO_REACHABILITY_ALLOWED_IPS", raising=False)
    monkeypatch.setenv("EXO_REACHABILITY_ALLOWED_CIDRS", "192.168.1.0/24")

    assert not is_probeable_interface(
        "en9", "169.254.49.224", interface_type="apple_usb_ncm"
    )


def test_probeable_interface_honors_allowed_cidrs(monkeypatch: MonkeyPatch):
    monkeypatch.delenv("EXO_REACHABILITY_ALLOWED_IPS", raising=False)
    monkeypatch.setenv("EXO_REACHABILITY_ALLOWED_CIDRS", "192.168.1.0/24")

    assert is_probeable_interface("en11", "192.168.1.63")
    assert not is_probeable_interface("en2", "192.168.0.2")
    assert not is_probeable_interface("bridge100", "192.168.2.1")


def test_probeable_interface_honors_allowed_ips(monkeypatch: MonkeyPatch):
    monkeypatch.setenv("EXO_REACHABILITY_ALLOWED_IPS", "192.168.1.63,192.168.1.120")
    monkeypatch.setenv("EXO_REACHABILITY_ALLOWED_CIDRS", "192.168.1.0/24")

    assert is_probeable_interface("en11", "192.168.1.63")
    assert is_probeable_interface("en0", "192.168.1.120")
    assert not is_probeable_interface("en0", "192.168.1.38")
    assert not is_probeable_interface("en16", "192.168.1.224")
