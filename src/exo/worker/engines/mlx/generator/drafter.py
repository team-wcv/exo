...
def make_drafter(
    *,
    ...
    if mode == "pipelined":
        ...
        # Validate target_peer_fanout shape early so a malformed
        # caller fails here, not deep inside the spec loop's broadcast
        # helpers. ``None`` is fine on every path (single-rank /
        # symmetric / test fakes); the broadcast helpers fall back to
        # ``mx_broadcast_int_list`` in that case.
