...
class RemoteTransport:
    ...
    def __init__(
        ...
        *,
        ...
        # socket-level error escapes (drafter rank crashed, peer
        # closed mid-frame, etc.). Once true, subsequent requests must
        # not start a new spec session on this transport: the wire is
        # unrecoverable until the runner is restarted (the master's
        # instance-deletion path tears the placement down within ~5s
        # of the drafter node leaving the topology). Callers consult
        # :attr:`is_failed` before constructing a
        # :class:`PipelinedModelDrafter`; the runner subprocess exits
        # via the spec-loop exception if the failure happens
        # mid-request (see ``_pipelined_speculative_step``'s abort
        # sentinel).
    ...
    def is_failed(self) -> bool:
        ...
        Set by the blocking wire helpers when an :class:`OSError`
        escapes (drafter rank crashed, peer closed mid-frame, etc.).
        Sticky: there is no in-place recovery -- the runner must be
        torn down (via the master's instance-deletion path) and a
        fresh transport built. Callers consult this flag before
        constructing a :class:`PipelinedModelDrafter`; if true the
        request degrades to non-speculative decoding for the
        remaining lifetime of the runner.
        """
    ...
    def _forward_blocking(
        ...
        try:
            ...
            # Drafts buffer is fixed-size at K + 1 (the upper bound of
            # any forward request); we slice to ``num_forwards`` here.
            drafts = recv_uint32_frame(self._sock, self._num_draft_tokens + 1)
        except OSError:
            # Drafter rank closed the socket / peer reset / broken
            # pipe. Mark the transport so subsequent
            # ``open_session`` calls fail fast and the runner can be
            # torn down (master-driven instance deletion) instead of
            # silently producing nothing on every speculative round.
            self._mark_failed()
            raise
