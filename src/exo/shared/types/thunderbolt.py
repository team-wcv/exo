...
class ThunderboltConnectivityData(BaseModel, extra="ignore"):
    ...
    def ident(self, ifaces: dict[str, str]) -> ThunderboltIdentifier | None:
        ...
        # macOS sometimes reports a receptacle tag (e.g. "Thunderbolt 1")
        # that has no matching networksetup hardware-port entry --
        # happens when a TB port is renumbered after the bridge service
        # is created. Skip the entry rather than killing the gatherer.
