...
def place_instance(
    ...
    # singleton total-memory expansion. These two checks are mutually
    # exclusive in practice -- the JACCL preflight only fires when
    # ``instance_meta == MlxJaccl`` (multi-node) and the
    # ``allow_single_node_total_memory`` expansion only fires for singleton
    # cycles, which were already downgraded to ``MlxRing`` by the block
    # above -- but we keep both unconditional so the invariant is encoded
    # in the code itself rather than in a comment about ordering.
    #
    # The drafter-multi-node degradation hint is emitted later (after
    # ``_select_drafter_placement``) so it only fires when an asymmetric
    # drafter rank was *not* chosen. Asymmetric placements (a separate
    # drafter rank attached to the parent ``mx.distributed`` group) DO
    # run speculative decoding across a multi-node target, so a
    # premature unconditional warning would mislead operators into
    # thinking JACCL clusters can't run drafters at all.
    if instance_meta == InstanceMeta.MlxJaccl:
...
def auto_place_prefill_siblings(
    *,
    ...
    decode_nodes: set[NodeId] = {
        nid for nid, _, _ in decode_instance.shard_assignments.shards
    }
